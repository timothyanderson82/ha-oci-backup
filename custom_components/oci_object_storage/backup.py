"""Backup platform for the OCI Object Storage integration."""
...
from . import OCIConfigEntry
from .const import CONF_BUCKET, DATA_BACKUP_AGENT_LISTENERS, DOMAIN

_LOGGER = logging.getLogger(__name__)
CACHE_TTL = 300

# S3 part size requirements: 5 MiB to 5 GiB per part, at most 10,000 parts.
# We set the threshold to 20 MiB to avoid too many parts (~265 for a 5.2 GB backup).
...
async def async_get_backup_agents(
    ...
    entries: list[OCIConfigEntry] = hass.config_entries.async_loaded_entries(DOMAIN)
    return [OCIBackupAgent(hass, entry) for entry in entries]
...
class OCIBackupAgent(BackupAgent):
    """Backup agent for the OCI Object Storage integration."""

    domain = DOMAIN

    def __init__(self, hass: HomeAssistant, entry: OCIConfigEntry) -> None:
        """Initialize the OCI agent."""
        super().__init__()
    ...
    async def async_upload_backup(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Upload the metadata file last; its presence marks a complete backup
    ...
    async def _upload_multipart(
        ...
        try:
            ...
            async for chunk in stream:
                ...
                # Upload parts of exactly MULTIPART_MIN_PART_SIZE_BYTES so that
                # all non-trailing parts have the same size
                ...
                # Compact the buffer once the consumed offset has grown large
                # enough, rather than copying after every part upload.
                if offset and offset >= MULTIPART_MIN_PART_SIZE_BYTES:
                    buffer = bytearray(buffer[offset:])
                    offset = 0

            # Upload the remaining buffer as the last part (no minimum size)
            ...
            # Abort so OCI does not keep (and bill for) the uploaded parts
            try:
    ...
    async def _list_metadata_keys(self) -> list[str]:
        """Return the keys of all metadata files, following pagination."""
    ...
    async def _list_backups(self) -> dict[str, AgentBackup]:
        ...
        backups = {}
        for key in await self._list_metadata_keys():
            try:
                ...
                _LOGGER.warning("Failed to process metadata file %s: %s", key, err)
                continue
